...
export default function ListWinners ({ route: { params } }) {
    const [data, setData] = useState([]);
    const [driver, setDriver] = useState('');

    const handleWinner = (item) => {
        params.navigation.navigate('SzczegółyWyniku', {
            details: item
        });
    };
    ...
    return (
        <View>
            ...
            <View style={styles.container1}>
                <Text style={styles.result}>Pozycja</Text>
            ...
            <FlatList 
                ...
                renderItem={({ item }) => (
                        <TouchableHighlight
                            onPress={() => handleWinner(item)}>
                            <View style={styles.container2}>
                                <Text style={styles.result1}>{item.position}</Text>
